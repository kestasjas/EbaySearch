    ...
    public class WebTest : WebDriver
    {
        ...
        void CheckCart()
        {
            ...
            try 
            {
                ...
                wait.Until(ExpectedConditions.ElementToBeClickable(asGuestButton));
                asGuestButton.Click();
                WaitForPageToLoad();
            }
            catch 
            {
                try
                {
                    driver.FindElement(By.XPath("//button[@aria-label='Close dialog']//svg[contains(@class,'Icon')]")).Click();
                }
                catch { }
